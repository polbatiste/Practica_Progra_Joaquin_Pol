...
def debug_animales(owner_id, animales):
   st.write("Debug info:")
   st.write(f"Owner ID: {owner_id}")
   st.write(f"Total animales: {len(animales)}")
   st.write("Todos los animales:", [f"{a['name']} (owner_id: {a['owner_id']})" for a in animales])
   animales_filtrados = [a for a in animales if a['owner_id'] == owner_id]
   st.write(f"Animales filtrados ({len(animales_filtrados)}):", [a['name'] for a in animales_filtrados])
...
def obtener_animales():
   try:
       ...
       if respuesta.status_code == 200:
           ...
           st.write(f"Animales obtenidos: {len(animales)}")
           return animales
       return []
   except:
...
def popup():
   ...
   with st.form("formulario_cita"):
       ...
       # Debug y selector de animal filtrado
       animal_id = None
       if owner_id:
           debug_animales(owner_id, animales)
